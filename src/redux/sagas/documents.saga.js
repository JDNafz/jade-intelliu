...
function* uploadUserDocuments(action) {
  ...
  try {
    ...
    const recentDoc = yield axios.get("/api/document/lowspec/recent");
    // console.log(
    //   "RECENT DOC BACK:",
    //   recentDoc.data[0].id,
    //   "standard:",
    //   standardId
    // );
    yield console.log("RECENT DOC DATA[0].id:", recentDoc.data[0]);
    yield put({
...
function* fetchDocsToSubmit(action) {
  ...
  try {
    ...
    console.log("bundled and ready:", response.data);
