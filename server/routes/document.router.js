...
const convertAndUploadXLDoc = require("../modules/XLModules/convertAndUploadXLDoc");
...
router.get("/lowspec", (req, res) => {
  pool
    ...
    .then((result) => {
      //if there is JSON data, make sure it also has XL data
      for (i = 0; i < result.rows.length; i ++){
        if (result.rows[i].download_file === null){
          convertAndUploadXLDoc(result.rows[i]);
        }
      }
